...
- **A new decoder needs a semantic assertion, not just a round trip.** `tests/fixture-semantics.test.ts` says what each fixture *means*, field by field; round-trip fidelity is blind to a decoder and encoder that are wrong in mirror-image ways. Derive expected values from the bytes and the spec by hand — a table captured from this package's own output pins its bugs as correct. Where a field can be corroborated by something independent (another block in the same EDID, the VIC table, the device's model number), assert that too.
- **`testedids.test.ts` requires a byte-for-byte re-encode.** Every auto-collected fixture must encode back to exactly its input; a fixture that legitimately cannot needs an entry in `EXPECTED_ENCODE_DIFFERENCES` naming the offsets and saying why the input is the wrong side. Do not widen that allowlist to make a failure go away — an unexplained offset is the signature of an encoder dropping something, which is how every silent-rewrite bug in this package looked.
