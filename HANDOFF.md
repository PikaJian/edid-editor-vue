...
| Release | [v0.1.2](https://github.com/PikaJian/edid-editor-vue/releases/tag/v0.1.2) is current — the Windows extension-block fix (#2). `main` is 3 commits ahead of that tag, all release-notes plumbing (#3) and docs, so nothing user-facing is unreleased. Older releases are superseded and say so: v0.1.0's Windows build lists every monitor ever attached (`33ef688`), v0.1.1's returns only base blocks (#2). |
