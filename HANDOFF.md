...
**The history was rewritten on 2026-08-17, so every commit SHA changed.** A private author email was replaced with `8335844+PikaJian@users.noreply.github.com` (`git filter-repo --mailmap`) just after the repository went public, and `main` plus all three tags were force-pushed. Consequences worth knowing: the SHAs cited in this document are post-rewrite and resolve, but **the PR pages for #1–#3 still name their original merge commits**, which are no longer reachable from `main` — a SHA from a PR page that does not resolve is that, not a lost commit. This clone's `user.email` is set locally to the noreply address; a clone made elsewhere inherits whatever the global config says.
